import re
text = '''E D U C A T I O N
Mobile
+(380) 99 795 842
Email
annmartyne@email.com

'''
# without_spaces = ''.join(text.split())
# print(without_spaces)
# phone_regexp = r'/\(?([0-9]{3})\)?([ .-]?)([0-9]{3})\2([0-9]{4})/'
# phone = re.search(phone_regexp, without_spaces)
# if phone:
#     print(phone.group(0))
# else:
#     print('phone not found')
email_regexp = r"(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])"
found_email = re.search(email_regexp, text)
if found_email:
    print(found_email.group(0))